...
exports.getFiles = function (container, options, callback) {
  ...
  exports.createMultipart=function(options,callback){
    ...
    var Params = {
      ...
      partSize = options.partSize || 1024 * 1024 * 5,
    };
  self.s3.create(Params,function(err,data){
    if(err){
      callback(err);
    }
    else{
  ...
  exports.uploadPart=function(options,callback){  
    var multipartMap = {
      Parts: []
    };
    var self = this,
      ...
      uploadedParts = 0,
      streamEnded = false,
      partSize = options.partSize; // Minimum 5MB per chunk (except the last part) http://docs.aws.amazon.com/AmazonS3/latest/API/mpUploadComplete.html
      chunkedStream = new storage.ChunkedStream(partSize),
      proxyStream = through(),
      
      _uploadPart = function (multipart, partParams) {
        self.s3._uploadPart(partParams, function(multiErr, mData) {
          if (multiErr) {
    ...
    self.s3.uploadPart(options, function(mpErr, multipart) {
      if (mpErr) {
      ...
      chunkedStream.on('data', function (data) {
        ...
        _uploadPart(multipart, partParams);
      });
  ...
  exports.abort=function(options,callback){
    var self=this;
    var abortOptions = {
      PartNumber: String(options.partNumber),
      UploadId: options.uploadId
    };
    self.s3.abort(abortOptions, function(err,data){
      if (err) {
        callback(err);
      } else {
        callback(null, data);
